...
| `yrt/runtime.py`: `parse_exe_mode`, `load_config() -> AppConfig`, `GitHubTarget`, `Session`, `bootstrap(ctx)` (loggers on the job's history file, service per mode), `finalize(ctx, session)` (token files / secret, quota summary, end marker, `copy_last_exe_log` per job file), `run_job(ctx, body) -> int` | `yrt/runtime.py`, `yrt/models.py`                           |
