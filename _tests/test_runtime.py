...
from yrt.constants import EXE_MODE_ACTION, EXE_MODE_LOCAL, PROCESS_END_MARKER, PROCESS_START_MARKER
...
def _session(exec_context, *, creds_b64=None, target=None):
    """Build a Session on the context's history file with the start marker already written."""
...
class TestBootstrap:
    ...
    @staticmethod
    def test_action_mode(exec_context, monkeypatch):
        ...
        assert _lines(exec_context.history_path) == [PROCESS_START_MARKER]
        assert utils.history is not before  # the youtube package now logs to the job's file

    @staticmethod
    def test_local_mode(exec_context):
        ...
        assert _lines(local_context.history_path) == [PROCESS_START_MARKER]
...
class TestFinalize:
    ...
    @staticmethod
    def test_local_mode_encodes_both_token_files(exec_context):
        """Test local mode re-encodes credentials.json and oauth.json."""
        local_context = runtime.ExecutionContext(**{**vars(exec_context), 'exe_mode': EXE_MODE_LOCAL})
        session = _session(local_context)
    ...
    @staticmethod
    def test_action_mode_updates_the_secret(exec_context):
        ...
        session = _session(exec_context, creds_b64='b64creds', target=target)
    ...
    @staticmethod
    def test_action_mode_without_creds_skips_the_secret(exec_context):
        """Test nothing is written back when the session carries no credentials."""
        session = _session(exec_context, creds_b64=None, target=runtime.GitHubTarget(repo='o/r', pat='t'))
    ...
    @staticmethod
    def test_logs_summary_then_end_marker_and_writes_last_exe(exec_context, quota_tracker):
        """Test the quota summary precedes the end marker and the last-exe log holds the whole run."""
        session = _session(exec_context, target=runtime.GitHubTarget(repo='o/r', pat='t'))
        ...
        assert _lines(exec_context.last_exe_path) == lines
...
class TestCopyLastExeLog:
    ...
    @staticmethod
    def test_keeps_only_the_last_run(tmp_path):
        ...
        assert _lines(last_exe) == [PROCESS_START_MARKER, 'new']
...
@pytest.mark.unit
class TestRunJob:
    """Test run_job() drives bootstrap -> body -> finalize and maps errors to exit codes."""

    @staticmethod
    def test_success_returns_zero(exec_context, quota_tracker, monkeypatch):
        """Test a clean run calls the body with the session and refreshes the last-exe log."""
        monkeypatch.setenv('GITHUB_REPOSITORY', 'owner/repo')
        monkeypatch.setenv('PAT', 'token')
        body = MagicMock()
        ...
        with (
            ...
            code = runtime.run_job(exec_context, body)

        assert code == 0
        ctx, session = body.call_args.args
        assert ctx is exec_context
        assert isinstance(session, runtime.Session)
        assert _lines(exec_context.last_exe_path)[-1] == PROCESS_END_MARKER

    @staticmethod
    def test_tracker_error_returns_one_and_keeps_last_exe(exec_context, quota_tracker, monkeypatch):
        """Test a YouTubeTrackerError is logged as fatal with the quota summary; the last-exe log is not rewritten."""
        monkeypatch.setenv('GITHUB_REPOSITORY', 'owner/repo')
        monkeypatch.setenv('PAT', 'token')
        exec_context.last_exe_path.write_text('previous run\n', encoding='utf-8')
        ...
        assert exec_context.last_exe_path.read_text(encoding='utf-8') == 'previous run\n'

    @staticmethod
    def test_bootstrap_failure_is_fatal(exec_context, quota_tracker, monkeypatch):
        ...
        assert any(
            line.startswith('Fatal error: Required environment variable') for line in _lines(exec_context.history_path)
        )
